...
export function initAuth() {
  ...
  function renderAccountsList() {
    ...
    accounts.forEach(function (acc) {
      ...
      li.innerHTML =
        '<button type="button" class="account-item-btn" data-id="' +
        acc.id +
        '">' +
        ...
        "</span>" +
        '<span class="account-item-date">Desde ' +
        acc.createdAt +
        "</span>" +
        "</button>" +
        '<button type="button" class="account-item-delete" data-id="' +
        acc.id +
    ...
    if (!list.dataset.listenerReady) {
      list.addEventListener("click", function (e) {
        ...
        if (loginBtn) {
          ...
          if (usernameInput) {
            usernameInput.value =
              loginBtn.querySelector(".account-item-name").textContent;
        ...
        if (deleteBtn) {
          ...
          const accName =
            deleteBtn.closest("li").querySelector(".account-item-name")
              ?.textContent || "esta cuenta";
  ...
  function showConfirmDelete(accId, accName) {
    ...
    panel.innerHTML =
      "<p>¿Desea ELIMINAR la cuenta <strong>" +
      escapeHTML(accName) +
      "</strong> y TODOS sus datos?</p>" +
    ...
    document
      .getElementById("auth-delete-yes")
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", function (e) {
      ...
      let hasError = false;
      if (!username) {
        setError("login-username-error", "Ingresá tu usuario.");
        hasError = true;
      }
      if (!password) {
        setError("login-password-error", "Ingresá tu contraseña.");
        hasError = true;
      }
      if (hasError) return;
  ...
  if (registerForm) {
    registerForm.addEventListener("submit", function (e) {
      ...
      if (!username || username.length < 3) {
        setError("register-username-error", "Mínimo 3 caracteres.");
        hasError = true;
      }
      if (!password || password.length < 4) {
        setError("register-password-error", "Mínimo 4 caracteres.");
        hasError = true;
      }
      if (password !== password2) {
        setError("register-password2-error", "Las contraseñas no coinciden.");
        hasError = true;
      }
      if (hasError) return;
